...
<#
.SYNOPSIS
  Waits for a restic-mount GitHub Release asset to appear and emits its URL + SHA256.
...
.DESCRIPTION
  ...
  manifest needs:
    ...
    {
      "tag":        "v0.1.0",
      "assetName":  "restic-mount-v0.1.0-windows-amd64.zip",
      "url":        "https://github.com/.../restic-mount-v0.1.0-windows-amd64.zip",
...
.PARAMETER Tag
  Release tag to wait for, e.g. "v0.1.0". Required.

.PARAMETER Repo
  GitHub repo in "owner/name" form. Defaults to "Maki-Daisuke/restic-mount-win".

.PARAMETER AssetPattern
  Asset filename to match. Defaults to "restic-mount-<Tag>-windows-amd64.zip".
  If not found, also tries "restic-mount-windows-amd64.zip".
...
.EXAMPLE
  ./scripts/wait-release.ps1 -Tag v0.1.0

.EXAMPLE
  $info = ./scripts/wait-release.ps1 -Tag v0.1.0 | ConvertFrom-Json
  $info.url
  $info.sha256
#>
...
param(
    ...
    [string]$Repo = 'Maki-Daisuke/restic-mount-win',
...
$displayPattern = if ($AssetPattern) { $AssetPattern } else { "restic-mount-$Tag-windows-amd64.zip (or restic-mount-windows-amd64.zip)" }
Write-Host "Waiting for release $Tag in $Repo (asset: $displayPattern, timeout: ${TimeoutSeconds}s)" -ForegroundColor Cyan

while ($true) {
    $now = Get-Date
    if ($now -gt $deadline) {
        Write-Error "Timed out after ${TimeoutSeconds}s waiting for $Tag asset $displayPattern."
        exit 1
    }
    ...
    if ($raw) {
        ...
        if ($release -and $release.assets) {
            ...
            if ($AssetPattern) {
                ...
                # Try tag-specific pattern first, fallback to untagged pattern
                $asset = $release.assets | Where-Object { $_.name -eq "restic-mount-$Tag-windows-amd64.zip" } | Select-Object -First 1
                if (-not $asset) {
                    $asset = $release.assets | Where-Object { $_.name -eq "restic-mount-windows-amd64.zip" } | Select-Object -First 1
                }
            }
